...
const useOpenAI = async (transcriptText: string): Promise<string | null> => {
    try {
        const prompt = "Retrieve the top 5 words from the provided interview text and present them in an array. Ensure the keywords are meaningful words, not words like 'the, is, as, with', etc.";
        ...
        if (
            ...
            const topKeywords = response.choices[0].message.content;
            return topKeywords;
        } else {
