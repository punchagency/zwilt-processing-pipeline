...
export default async function getInterviewAssessmentToProcess(
  ...
  while (hasMore) {
    const query: any = {
      ...
      // ]
      $or: [
        { $expr: { $lt: ['$interviewReel.noOfVideosUsedForReel', { $size: '$categories' }] } },
        { 'interviewReel.noOfVideosUsedForReel': { $exists: false } }
      ]
    };
    ...
    if (matchFound) {
      // Filter out categories without valid transcript.words
      interviewAssessment.categories = categories.filter(category => {
        const responses = category.assessmentResponse as AssessmentResponse[];
        return responses.some(response => response.transcript && response.transcript.words.length >= 2);
      });
